...
<body>
    ...
    <script>
    function goBack(){
        window.location.href="<?=base_url();?>index.php/Ctrl_list/event";
    }
    var $ = jQuery.noConflict();
    $("#title").html("Admin | Delete Event");
    $(document).ready(function() {
        $('.breadcrumbs .page-title h1').html('Delete Event');
        ...
        $('.breadcrumbs .text-right li').eq(2).html('Delete');
          $("#submit").on("click",function(){
            $.ajax({
            url:"<?=base_url();?>index.php/Ctrl_functions/delEvent/<?=$id?>",
            type:"POST",
            success:function(result){
            if(result != '0'){
                window.location.href="<?=base_url();?>index.php/Ctrl_list/event/deleted";
            }
            else{
