...
<div class="container">
    ...
    <br>
        ...
        <table style="display:inline-block;width:75%; max-width: 80%;" class="table table-borderless">
          ...
          <tr><td class="formlabel">Front Page: </td><td><form id="imgform" method="POST" action="<?=base_url()?>index.php/Ctrl_publications/addChapterFrontPage"><input type="file" name="front" id="front" class="form-control"></form></td></tr>
        ...
        <tr class="typelink active"><td><a href="<?=base_url()?>index.php/Ctrl_tform/addChapter/"><i class="fa fa-arrow-right" aria-hidden="true"></i> Chapters in Books</a></td></tr>
    ...
    $("#submit").click(function(e){
      ...
      if($("#title").val() == "" || $("#scholar").val() == "" || $("#dept option:selected").val() == "0" || $("#chap").val() == "" || $("#publisher").val() == "" || $("#front").val() == "" || $("#link").val() == ""){
        alert("Please fill all the values!");
        return;
      }else{
        $.ajax({
          url: "<?=base_url();?>index.php/Ctrl_publications/addChapter/",
          type: "POST",
          data: {email: $("#email").val(), title: $("#title").val(), chapter: $("#chap").val(), link: $("#link").val(), dept: $('#dept option:selected').val(), publisher: $('#publisher').val(), year: $('#year option:selected').val(), fname: $('#fname').html(), scholar: $("#scholar").val()},
          success:function(result){
            if(result == '0'){
    ...
    $("body").on("click", ".editme", function(){
        ...
        $.ajax({
          url: "<?=base_url();?>index.php/Ctrl_publications/deleteChapter",
          type: "POST",
