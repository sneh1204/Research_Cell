...
<div class="container">
    ...
    <br>
        ...
        <table style="display:inline-block;width:75%; max-width: 80%;" class="table table-borderless">
          ...
          <tr><td class="formlabel">Certi Image: </td><td><form id="imgform" method="POST" action="<?=base_url()?>index.php/Ctrl_publications/addStudentCertiImage"><input type="file" name="cimage" id="cimage" class="form-control"></form></td></tr>
        ...
        <tr class="typelink active"><td><a href="<?=base_url()?>index.php/Ctrl_sform/addStudentCerti/"><i class="fa fa-arrow-right" aria-hidden="true"></i> Certification</a></td></tr>
        </table>
        </div>
        <div class="row">
    ...
    $("#submit").click(function(e){
      ...
      if($("#name").val() == "" || $("#dept option:selected").val() == "0" || $("#cname").val() == "" || $("#detail").val() == "" || $("#cimage").val() == "" || $("#date").val() == ""){
        alert("Please fill all the values!");
        return;
      }else{
        $.ajax({
          url: "<?=base_url();?>index.php/Ctrl_publications/addStudentCerti/",
          type: "POST",
          data: {email: $("#email").val(), name: $("#name").val(), cname: $("#cname").val(), date: $("#date").val(), dept: $('#dept option:selected').val(), detail: $('#detail').val(), fname: $('#fname').html(), cyear: $("#cyear").val()},
          success:function(result){
            if(result == '0'){
    ...
    $("body").on("click", ".editme", function(){
        ...
        $.ajax({
          url: "<?=base_url();?>index.php/Ctrl_publications/deleteStudentCerti",
          type: "POST",
