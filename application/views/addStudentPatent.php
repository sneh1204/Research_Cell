...
<div class="container">
    ...
    <br>
        ...
        <tr class="typelink active"><td><a href="<?=base_url()?>index.php/Ctrl_sform/addStudentPatent/"><i class="fa fa-arrow-right" aria-hidden="true"></i> Patent</a></td></tr>
    ...
    $("#submit").click(function(e){
      if($("#email").val() == "" || $("#status option:selected").val() == "0" || $("#dept option:selected").val() == "0" || $("#patentid").val() == "" || $("#title").val() == "" || $("#inventor").val() == "" || $("#applicant").val() == ""){
        ...
        $.ajax({
          url: "<?=base_url();?>index.php/Ctrl_publications/addStudentPatent/",
          type: "POST",
          data: {email: $("#email").val(), patentid: $("#patentid").val(), title: $("#title").val(), applicant: $("#applicant").val(), inventor: $("#inventor").val(), dept: $("#dept option:selected").val(), link: $("#link").val(), status: $('#status option:selected').val(), fdate: $('#fdate').val(), pdate: $('#pdate').val(), gdate: $('#gdate').val(), fname: $('#fname').html(), cyear: $('#cyear').val()},
          success:function(result){
            if(result == '0'){
    ...
    $("body").on("click", ".editme", function(){
        ...
        $.ajax({
          url: "<?=base_url();?>index.php/Ctrl_publications/deleteStudentPatent",
          type: "POST",
