...
<div class="container">
    ...
    <div class="col-md-12 animate-box">
        ...
        <span style="font-weight: bold;font-size: 20px;color:black;padding-top:10px;">Add Book</span>
    ...
    <br>
        ...
        <table style="display:inline-block;width:75%; max-width: 80%;" class="table table-borderless">
          ...
          <tr><td class="formlabel">Role: </td><td><select name="role" id="role" class="form-control"><option value="0" selected="selected">Select Role</option><option value="Author">Author</option><option value="Editor">Editor</option><option value="Reviewer">Reviewer</option></select></td>
          ...
          <tr><td class="formlabel">Front Page: </td><td><form id="imgform" method="POST" action="<?=base_url()?>index.php/Ctrl_publications/addStudentFrontPage"><input type="file" name="front" id="front" class="form-control"></form></td></tr>
        ...
        <tr class="typelink active"><td><a href="<?=base_url()?>index.php/Ctrl_sform/addStudentBook/"><i class="fa fa-arrow-right" aria-hidden="true"></i> Books</a></td></tr>
        ...
        <tr style="color:black;"><th>ID</th><th>Front Page</th><th>Title</th><th>Role</th><th>Publisher</th><th>Year</th><th>Link</th><th>Update</th></tr>
        <?php
        foreach($fbooks as $sid => $data){
          $rlength = $sid+1;
          $src = base_url()."illustrations/booksfront/".$data->frontpage;
          echo '<tr style="color:grey;" class="subrow" id="tr'.$data->id.'"><td class="sub" id="srno'.$rlength.'">'.$data->id.'</td><td class="sub"><img src="'.$src.'" style="height:100px;width:80px;"></img></td><td class="sub">'.$data->title.'</td><td class="sub">'.$data->role.'</td><td class="sub">'.$data->publisher.'</td><td class="sub">'.$data->year.'</td><td class="sub">'.$data->link.'</td><td class="sub"><a href="#" class="editme" id="edit'.$rlength.'" style="color:green;">Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="deleteme" id="delete'.$rlength.'" style="color:red;">Delete</a></td></tr>';
        }
        ?>
        </table>
    </div>
    ...
    $("#submit").click(function(e){
      ...
      if($("#title").val() == "" || $("#dept option:selected").val() == "0" || $("#role").val() == 0 || $("#publisher").val() == "" || $("#front").val() == "" || $("#link").val() == ""){
        alert("Please fill all the values!");
        return;
      }else{
        $.ajax({
          url: "<?=base_url();?>index.php/Ctrl_publications/addStudentBook/",
          type: "POST",
          data: {email: $("#email").val(), title: $("#title").val(), role: $("#role").val(), link: $("#link").val(), dept: $('#dept option:selected').val(), publisher: $('#publisher').val(), year: $('#year option:selected').val(), fname: $('#fname').html(), cyear: $("#cyear").val()},
          success:function(result){
            if(result == '0'){
              alert("Error while inserting!");
              return;
            }else{
              $("#submitted").append('<tr style="color:grey;" class="subrow" id="tr'+result+'"><td class="sub" id="srno'+result+'">'+result+'</td><td class="sub"></td><td class="sub">'+$("#title").val()+'</td><td class="sub">'+$('#role option:selected').text()+'</td><td class="sub">'+$('#publisher').val()+'</td><td class="sub">'+$('#year option:selected').val()+'</td><td class="sub">'+$('#link').val()+'</td><td class="sub"><a href="#" class="editme" id="edit'+result+'" style="color:green;">Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="deleteme" id="delete'+result+'" style="color:red;">Delete</a></td></tr>');
    ...
    $("body").on("click", ".editme", function(){
        ...
        $.ajax({
          url: "<?=base_url();?>index.php/Ctrl_publications/deleteStudentBook",
          type: "POST",
...
<div class="gototop js-top">
  ...
  <script>
  $("#pagetitle").html("Research Cell | Add Book");
