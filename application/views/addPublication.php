...
<div class="container">
    ...
    <strong>Please update your publication's date and other details! Click Edit.</strong>
    </div>
    <?php
    }
    ?>
    ...
    <br>
        ...
        <table style="display:inline-block;width:75%; max-width: 80%;" class="table table-borderless">
          ...
          <tr><td class="formlabel" id="typename">Journal Name: </td><td><input type="text" name="journal" id="journal" class="form-control" placeholder="Journal Name" max-length="200"></td>
          <td><label style="color:black;"><input type="checkbox" name="with" id="with" class="form-control" style="height:20px;width:20px;float:right;"/>With Student: &nbsp;&nbsp;&nbsp;&nbsp;</label></td><td class="formlabel"></td>
          </tr>
        ...
        <tr class="typelink active"><td><a href="<?=base_url()?>index.php/Ctrl_tform/addPublication/"><i class="fa fa-arrow-right" aria-hidden="true"></i> Journal/Conference</a></td></tr>
    ...
    $("#submit").click(function(e){
      var ch = $("#with").prop('checked');
      ch = (ch == true) ? "true" : "false";
      if($("#email").val() == "" || $("#month").val() == "" || $("#dept").val() == "0" || $("#authors").val() == "" || $("#journal").val() == "" || $("#title").val() == "" || $("#doi").val() == "" || $("#link").val() == ""){
        ...
        $.ajax({
          url: "<?=base_url();?>index.php/Ctrl_publications/addPaper/",
          type: "POST",
          data: {email: $("#email").val(), authors: $("#authors").val(), journal: $("#journal").val(), title: $("#title").val(), doi: $("#doi").val(), link: $("#link").val(), dept: $('#dept').val(), type: $('#type option:selected').val(), year: year, fname: $('#fname').html(), scholar: $('#scholar').val(), month: month, student: ch},
          success:function(result){
            if(result == '0'){
    ...
    $("body").on("click", ".editme", function(){
        ...
        $.ajax({
          url: "<?=base_url();?>index.php/Ctrl_publications/deletePaper",
          type: "POST",
